...
public class ApiConnection extends EClientSocket {
	...
	public ApiConnection(String host, int port, int clientId, AnyWrapper wrapper, ILogger inLogger, ILogger outLogger) {
		super(host, port, clientId, wrapper);
		m_inLogger = inLogger;
	}
	...
	public synchronized void placeOrder(NewContract contract, NewOrder order) {
		...
		// ApiController requires TWS 932 or higher; this limitation could be
		// removed if needed
		if (serverVersion() < 66) {

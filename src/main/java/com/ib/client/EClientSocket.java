...
public class EClientSocket {
	private final static Logger log = LoggerFactory.getLogger(EClientSocket.class);
	...
	public boolean isConnected() {
		...
		if (socket.isConnected())
			try {// 发送心跳包
				...
				log.error("send Heartbeat error,return false");
				return false;
			}
	...
	public EClientSocket(String host, int port, int clientId, AnyWrapper anyWrapper) {
		this.host = host;
		this.port = port;
	...
	private synchronized void eConnect() throws IOException {
	 ...
	 */
	public synchronized void reqHistoricalData(int tickerId, Contract contract, String endDateTime, String durationStr, String barSizeSetting,
	...
	public synchronized void placeOrder(int id, Contract contract, Order order) {
		...
		try {
			...
			if (m_serverVersion >= MIN_SERVER_VER_SMART_COMBO_ROUTING_PARAMS && BAG_SEC_TYPE.equalsIgnoreCase(contract.m_secType)) {
				Vector<TagValue> smartComboRoutingParams = order.m_smartComboRoutingParams;
			...
			if (m_serverVersion >= MIN_SERVER_VER_ALGO_ORDERS) {
				...
				if (!IsEmpty(order.m_algoStrategy)) {
					Vector<TagValue> algoParams = order.m_algoParams;
			...
			if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
				...
				Vector<TagValue> orderMiscOptions = order.m_orderMiscOptions;

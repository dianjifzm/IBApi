...
public class EReader extends Thread {
	...
	private final static Logger log = LoggerFactory.getLogger(EReader.class);
	...
	protected boolean processMsg(int msgId) throws IOException {
		...
		case TICK_EFP: {
			...
			eWrapper().tickEFP(tickerId, tickType, basisPoints, formattedBasisPoints, impliedFuturesPrice, holdDays, futureExpiry, dividendImpact,
					dividendsToExpiry);
			break;
		}
